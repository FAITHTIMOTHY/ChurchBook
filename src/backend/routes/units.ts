...
import { collections, saveDB, Unit } from '../db/db.js';
...
router.post('/', authMiddleware, requireRole(['Pastor', 'HOD']), (req: AuthenticatedRequest, res: Response) => {
...
router.delete('/:id', authMiddleware, requireRole(['Pastor']), (req: AuthenticatedRequest, res: Response) => {
  const tenantId = req.tenantId!;
  ...
  members.forEach(m => {
    if (m.unit_id === unitId) m.unit_id = null;
  });

  const inventory = collections.getInventory();
  inventory.forEach(i => {
    if (i.unit_id === unitId) i.unit_id = null;
  });

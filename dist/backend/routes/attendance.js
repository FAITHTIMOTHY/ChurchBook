...
router.get('/sheet', authMiddleware, requireRole(['Usher', 'Pastor']), (req, res) => {
    const tenantId = req.tenantId;
    const serviceDate = req.query.date || new Date().toISOString().split('T')[0];
    const serviceName = req.query.serviceName || 'Sunday Worship';
    const members = collections.getMembers().filter(m => m.tenant_id === tenantId);
    const attendanceRecords = collections.getAttendance().filter(a => a.tenant_id === tenantId &&
...
// Restricted to Ushers and Pastors
router.post('/mark', authMiddleware, requireRole(['Usher', 'Pastor']), (req, res) => {
    ...
    const existingRecordIdx = attendance.findIndex(a => a.tenant_id === tenantId &&
...
// Restricted to Ushers and Pastors
router.post('/close-service', authMiddleware, requireRole(['Usher', 'Pastor']), (req, res) => {
    const currentUser = req.user;
    const tenantId = req.tenantId;
    ...
    for (const m of members) {
        const hasRecord = attendance.some(a => a.tenant_id === tenantId &&
            a.member_id === m.id &&
            a.service_date === serviceDate &&
            a.service_name === serviceName);
...
router.get('/stats', authMiddleware, requireRole(['Pastor', 'HOD', 'FollowUp']), (req, res) => {
    const tenantId = req.tenantId;

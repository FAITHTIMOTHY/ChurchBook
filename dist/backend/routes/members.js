...
function pruneMemberData(member, currentUser) {
...
router.put('/:id', authMiddleware, (req, res) => {
    ...
    if (phone !== undefined)
        targetMember.phone = phone;
    if (address !== undefined)
        targetMember.address = address;
    if (profile_picture_url !== undefined)
        targetMember.profile_picture_url = profile_picture_url;
    if (dob_month !== undefined)
        targetMember.dob_month = Number(dob_month);
    if (dob_day !== undefined)
        targetMember.dob_day = Number(dob_day);
    if (role !== undefined)
        targetMember.role = role;
    if (position !== undefined)
        targetMember.position = position;
    if (unit_id !== undefined)
        targetMember.unit_id = unit_id || null;
    if (status !== undefined)
